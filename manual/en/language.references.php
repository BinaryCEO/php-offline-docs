...
<div class="phpcode"><code><span class="html">in the example below, you would get the same result if you change the function to something like:<br /><br />function test_ref(&amp;$arr) {<br />   $time = time();<br />   $size = sizeof($arr);       // &lt;--- this makes difference...<br />   for($n=0; $n&lt;$size; $n++) {<br />       $x = 1;<br />   }<br /><br />   echo "&lt;br /&gt;The function using a reference took ".(time() - $time)." s";<br />}</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">If you want to know the reference count of a particular variable, then here's a function that makes use of debug_zval_dump() to do so:<br /><br /><span class="default">&lt;?php<br /></span><span class="keyword">function </span><span class="default">refcount</span><span class="keyword">(</span><span class="default">$var</span><span class="keyword">)<br />{<br />    </span><span class="default">ob_start</span><span class="keyword">();<br />    </span><span class="default">debug_zval_dump</span><span class="keyword">(</span><span class="default">$var</span><span class="keyword">);<br />    </span><span class="default">$dump </span><span class="keyword">= </span><span class="default">ob_get_clean</span><span class="keyword">();<br /><br />    </span><span class="default">$matches </span><span class="keyword">= array();<br />    </span><span class="default">preg_match</span><span class="keyword">(</span><span class="string">'/refcount\(([0-9]+)/'</span><span class="keyword">, </span><span class="default">$dump</span><span class="keyword">, </span><span class="default">$matches</span><span class="keyword">);<br /><br />    </span><span class="default">$count </span><span class="keyword">= </span><span class="default">$matches</span><span class="keyword">[</span><span class="default">1</span><span class="keyword">];<br /><br />    </span><span class="comment">//3 references are added, including when calling debug_zval_dump()<br />    </span><span class="keyword">return </span><span class="default">$count </span><span class="keyword">- </span><span class="default">3</span><span class="keyword">;<br />}<br /></span><span class="default">?&gt;<br /></span><br />debug_zval_dump() is a confusing function, as explained in its documentation, as among other things, it adds a reference count when being called as there is a reference within the function. refcount() takes account of these extra references by subtracting them for the return value.<br /><br />It's also even more confusing when dealing with variables that have been assigned by reference (=&amp;), either on the right or left side of the assignment, so for that reason, the above function doesn't really work for those sorts of variables. I'd use it more on object instances.<br /><br />However, even taking into account that passing a variable to a function adds one to the reference count; which should mean that calling refcount() adds one, and then calling debug_zval_dump() adds another, refcount() seems to have aquired another reference from somewhere; hence subtracting 3 instead of 2 in the return line. Not quite sure where that comes from.<br /><br />I've only tested this on 5.3; due to the nature of debug_zval_dump(), the results may be completely different on other versions.</span></code></div>
  ...
  <strong class="user"><em>ivan at mailinator dot com</em></strong></a><a class="genanchor" href="#87532"> &para;</a><div class="date" title="2008-12-09 05:09"><strong>16 years ago</strong></div>
  <div class="text" id="Hcom87532">
<div class="phpcode"><code><span class="html">A little gotcha (be careful with references!):<br /><br /><span class="default">&lt;?php<br />$arr </span><span class="keyword">= array(</span><span class="string">'a'</span><span class="keyword">=&gt;</span><span class="string">'first'</span><span class="keyword">, </span><span class="string">'b'</span><span class="keyword">=&gt;</span><span class="string">'second'</span><span class="keyword">, </span><span class="string">'c'</span><span class="keyword">=&gt;</span><span class="string">'third'</span><span class="keyword">);<br />foreach (</span><span class="default">$arr </span><span class="keyword">as &amp;</span><span class="default">$a</span><span class="keyword">); </span><span class="comment">// do nothing. maybe?<br /></span><span class="keyword">foreach (</span><span class="default">$arr </span><span class="keyword">as </span><span class="default">$a</span><span class="keyword">);  </span><span class="comment">// do nothing. maybe?<br /></span><span class="default">print_r</span><span class="keyword">(</span><span class="default">$arr</span><span class="keyword">);<br /></span><span class="default">?&gt;<br /></span>Output:<br /><br />Array<br />(<br />    [a] =&gt; first<br />    [b] =&gt; second<br />    [c] =&gt; second<br />)<br /><br />Add 'unset($a)' between the foreachs to obtain the 'correct' output:<br /><br />Array<br />(<br />    [a] =&gt; first<br />    [b] =&gt; second<br />    [c] =&gt; third<br />)</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">I found a very useful summary of how references work in PHP4 (and some of the common pitfalls) in this article: <a href="http://www.obdev.at/developers/articles/00002.html" rel="nofollow" target="_blank">http://www.obdev.at/developers/articles/00002.html</a><br /><br />It deals with some subtle situations and I recommend it to anyone having difficulty with their references.</span></code></div>
  ...
  <div class="note" id="114620">  <div class="votes">
    ...
    <div class="tally" id="V114620" title="65% like this...">
    12
    </div>
  </div>
...
<div class="phpcode"><code><span class="html">Simply put, here's an example of what referencing IS:<br /><br /><span class="default">&lt;?php<br />    $foo  </span><span class="keyword">= </span><span class="default">5</span><span class="keyword">;<br />    </span><span class="default">$bar </span><span class="keyword">= &amp;</span><span class="default">$foo</span><span class="keyword">;<br />    </span><span class="default">$bar</span><span class="keyword">++;<br />    <br />    echo </span><span class="default">$foo</span><span class="keyword">;<br /></span><span class="default">?&gt;<br /></span><br />The above example will output the value 6, because $bar references the value of $foo, therefore, when changing $bar's value, you also change $foo's value too.</span></code></div>
  ...
  <div class="note" id="84088">  <div class="votes">
    ...
    <div class="tally" id="V84088" title="65% like this...">
    9
    </div>
  </div>
...
<div class="phpcode"><code><span class="html">It's strange that function definition AND call to the same function must have "&amp;" before them. <br /><br />$arr = array();<br />$ref =&amp; oras($arr['blah'], array());<br />$ref []= "via ref";<br />print_r($arr);<br /><br />/* result<br />Array<br />(<br />    [blah] =&gt; Array<br />        (<br />            [0] =&gt; via ref<br />        )<br /><br />)<br />*/<br /><br />// perl like ||=<br />function &amp;oras (&amp;$v, $new) {<br />  $v or $v = $new;<br />  return $v;<br />}</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">There is no built in method (yet) to check if two variables are references to the same piece of data, but you can do a "reference sniff" test. This is rarely needed, but can be very useful. The function bellow is a slightly modified version of this technique I saw in a forum regarding this comparison limitation.<br /><br /><span class="default">&lt;?php<br /></span><span class="keyword">function </span><span class="default">is_ref_to</span><span class="keyword">(&amp;</span><span class="default">$a</span><span class="keyword">, &amp;</span><span class="default">$b</span><span class="keyword">)<br />{<br />    </span><span class="default">$t </span><span class="keyword">= </span><span class="default">$a</span><span class="keyword">;<br />    if(</span><span class="default">$r</span><span class="keyword">=(</span><span class="default">$b</span><span class="keyword">===(</span><span class="default">$a</span><span class="keyword">=</span><span class="default">1</span><span class="keyword">))){ </span><span class="default">$r </span><span class="keyword">= (</span><span class="default">$b</span><span class="keyword">===(</span><span class="default">$a</span><span class="keyword">=</span><span class="default">0</span><span class="keyword">)); }<br />    </span><span class="default">$a </span><span class="keyword">= </span><span class="default">$t</span><span class="keyword">;<br />    return </span><span class="default">$r</span><span class="keyword">;<br />}<br /><br /></span><span class="default">$varA </span><span class="keyword">= </span><span class="default">1</span><span class="keyword">;<br /></span><span class="default">$varB </span><span class="keyword">= </span><span class="default">$varA</span><span class="keyword">;<br /></span><span class="default">$varC </span><span class="keyword">=&amp;</span><span class="default">$varA</span><span class="keyword">;<br /><br /></span><span class="default">var_dump</span><span class="keyword">( </span><span class="default">is_ref_to</span><span class="keyword">(</span><span class="default">$varA</span><span class="keyword">, </span><span class="default">$varB</span><span class="keyword">) ); </span><span class="comment">// bool(false)<br /></span><span class="default">var_dump</span><span class="keyword">( </span><span class="default">is_ref_to</span><span class="keyword">(</span><span class="default">$varA</span><span class="keyword">, </span><span class="default">$varC</span><span class="keyword">) ); </span><span class="comment">// bool(true)<br /></span><span class="default">?&gt;<br /></span><br />The test above uses a two step process to be 100% generic.<br />But if you are sure the variables being tested will not be a certain value, example null, then use that value to allow a one step check.<br /><br /><span class="default">&lt;?php<br /></span><span class="keyword">function </span><span class="default">is_ref_to_1step</span><span class="keyword">(&amp;</span><span class="default">$a</span><span class="keyword">, &amp;</span><span class="default">$b</span><span class="keyword">)<br />{<br />    </span><span class="default">$t </span><span class="keyword">= </span><span class="default">$a</span><span class="keyword">;<br />    </span><span class="default">$r</span><span class="keyword">=(</span><span class="default">$b</span><span class="keyword">===(</span><span class="default">$a</span><span class="keyword">=</span><span class="default">null</span><span class="keyword">));<br />    </span><span class="default">$a </span><span class="keyword">= </span><span class="default">$t</span><span class="keyword">;<br />    return </span><span class="default">$r</span><span class="keyword">;<br />}<br /></span><span class="default">?&gt;</span></span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">I tried to create an array with n depth using a recursive function passing array references around. So far I haven't had much luck and I couldn't find anything on the web. So I ended up using eval() and it seems to work well:<br /><span class="default">&lt;?php<br />      </span><span class="keyword">foreach(</span><span class="default">array_keys</span><span class="keyword">(</span><span class="default">$this</span><span class="keyword">-&gt;</span><span class="default">quantity_array</span><span class="keyword">) AS </span><span class="default">$key</span><span class="keyword">){<br />        if(</span><span class="default">$this</span><span class="keyword">-&gt;</span><span class="default">quantity_array</span><span class="keyword">[</span><span class="default">$key</span><span class="keyword">] &gt; </span><span class="default">0</span><span class="keyword">){<br />          </span><span class="default">$combinations </span><span class="keyword">= </span><span class="default">explode</span><span class="keyword">(</span><span class="string">'-'</span><span class="keyword">, </span><span class="default">$key</span><span class="keyword">);<br />          </span><span class="default">$eval_string </span><span class="keyword">= </span><span class="string">'$eval_array'</span><span class="keyword">;<br />          foreach(</span><span class="default">array_keys</span><span class="keyword">(</span><span class="default">$combinations</span><span class="keyword">) AS </span><span class="default">$key2</span><span class="keyword">){<br />            </span><span class="default">$option_key_value </span><span class="keyword">= </span><span class="default">explode</span><span class="keyword">(</span><span class="string">'_'</span><span class="keyword">, </span><span class="default">$combinations</span><span class="keyword">[</span><span class="default">$key2</span><span class="keyword">]);<br />            </span><span class="default">$eval_string </span><span class="keyword">.= </span><span class="string">'['</span><span class="keyword">.</span><span class="default">$option_key_value</span><span class="keyword">[</span><span class="default">0</span><span class="keyword">].</span><span class="string">']['</span><span class="keyword">.</span><span class="default">$option_key_value</span><span class="keyword">[</span><span class="default">1</span><span class="keyword">].</span><span class="string">']'</span><span class="keyword">;<br />          }<br />          </span><span class="default">$eval_string </span><span class="keyword">.= </span><span class="string">' = '</span><span class="keyword">.</span><span class="default">$this</span><span class="keyword">-&gt;</span><span class="default">quantity_array</span><span class="keyword">[</span><span class="default">$key</span><span class="keyword">].</span><span class="string">';'</span><span class="keyword">;<br />          eval(</span><span class="default">$eval_string</span><span class="keyword">);<br />        }<br />      }<br /></span><span class="default">?&gt;<br /></span><br />This produces an n dimensional array that will be available in the $eval_array variable. Hope it helps somebody!</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">Responding to Slava Kudinov.  The only reason why your script takes longer when you pass by reference is that you do not at all modify the array that your passing to your functions.  If you do that the diffrences in execution time will be a lot smaller.  In fact  passing by reference will be faster if just by a little bit.</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">In addition to the note made by "Francis dot a at gmx dot net" you should not normally be using a function such as sizeof() or count() in a control structure such as FOR because the same value is being calculated repeatedly for each iteration. This can slow things down immensely, regardless of whether you pass by value or reference.<br /><br />It is generally much better to calculate the static values before the defining the looping control structure.<br /><br />Example:<br /><br /><span class="default">&lt;?php<br /><br />$intSize </span><span class="keyword">= </span><span class="default">sizeof</span><span class="keyword">(</span><span class="default">$arrData</span><span class="keyword">);<br /><br />for(</span><span class="default">$i </span><span class="keyword">= </span><span class="default">0</span><span class="keyword">; </span><span class="default">$i </span><span class="keyword">&lt; </span><span class="default">$intSize</span><span class="keyword">; </span><span class="default">$n</span><span class="keyword">++) {<br />    </span><span class="comment">// Do stuff<br /></span><span class="keyword">}<br /><br /></span><span class="default">?&gt;</span></span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">This is to show the affect of changing property of object A through another object B when you pass object A as a property of another object B.<br /><br /><span class="default">&lt;?php<br /> </span><span class="comment">// data class to be passed to another class as an object<br /> </span><span class="keyword">class </span><span class="default">A</span><span class="keyword">{<br />     public </span><span class="default">$info</span><span class="keyword">;<br />    function </span><span class="default">__construct</span><span class="keyword">(){<br />        </span><span class="default">$this</span><span class="keyword">-&gt;</span><span class="default">info </span><span class="keyword">= </span><span class="string">"eeee"</span><span class="keyword">;<br />    }<br /> }<br /> <br /> </span><span class="comment">// B class to change the info in A obj<br /> </span><span class="keyword">class </span><span class="default">B_class</span><span class="keyword">{<br />     public </span><span class="default">$A_obj</span><span class="keyword">;<br />    <br />    function </span><span class="default">__construct</span><span class="keyword">(</span><span class="default">$A_obj</span><span class="keyword">){<br />        </span><span class="default">$this</span><span class="keyword">-&gt;</span><span class="default">A_obj </span><span class="keyword">=  </span><span class="default">$A_obj</span><span class="keyword">;<br />    }<br />    public function </span><span class="default">change</span><span class="keyword">(</span><span class="default">$newVal</span><span class="keyword">){<br />        </span><span class="default">$this</span><span class="keyword">-&gt;</span><span class="default">A_obj</span><span class="keyword">-&gt;</span><span class="default">info </span><span class="keyword">= </span><span class="default">$newVal</span><span class="keyword">;<br />    }<br /> }<br /><br /> </span><span class="comment">// create data object from the A<br /> </span><span class="default">$A_obj </span><span class="keyword">= new </span><span class="default">A</span><span class="keyword">();<br /> </span><span class="comment">// print the info property<br /> </span><span class="keyword">echo </span><span class="string">'A_obj info: ' </span><span class="keyword">. </span><span class="default">$A_obj</span><span class="keyword">-&gt;</span><span class="default">info </span><span class="keyword">. </span><span class="string">'&lt;br/&gt;'</span><span class="keyword">;<br /> <br /> </span><span class="comment">// create the B object and pass the A_obj we created above<br /> </span><span class="default">$B_obj </span><span class="keyword">= new </span><span class="default">B_class</span><span class="keyword">(</span><span class="default">$A_obj</span><span class="keyword">);<br /> </span><span class="comment">// print the info property through the B object to make sure it has the same value 'eeee'<br /> </span><span class="keyword">echo </span><span class="string">'B_obj info: ' </span><span class="keyword">. </span><span class="default">$B_obj</span><span class="keyword">-&gt;</span><span class="default">A_obj</span><span class="keyword">-&gt;</span><span class="default">info </span><span class="keyword">. </span><span class="string">'&lt;br/&gt;'</span><span class="keyword">;<br /> <br /> </span><span class="comment">// chage the info property<br /> </span><span class="default">$B_obj</span><span class="keyword">-&gt;</span><span class="default">change</span><span class="keyword">(</span><span class="string">'xxxxx'</span><span class="keyword">);<br /> </span><span class="comment">// print the info property through the B object to make sure it changed the value to 'xxxxxx'<br /> </span><span class="keyword">echo </span><span class="string">'B_obj info after change: ' </span><span class="keyword">. </span><span class="default">$B_obj</span><span class="keyword">-&gt;</span><span class="default">A_obj</span><span class="keyword">-&gt;</span><span class="default">info </span><span class="keyword">. </span><span class="string">'&lt;br/&gt;'</span><span class="keyword">;<br /> </span><span class="comment">// print the info property from the A_obj to see if the change through B_obj has affected it<br /> </span><span class="keyword">echo </span><span class="string">'A_obj info: ' </span><span class="keyword">. </span><span class="default">$A_obj</span><span class="keyword">-&gt;</span><span class="default">info </span><span class="keyword">. </span><span class="string">'&lt;br/&gt;'</span><span class="keyword">;<br /><br /></span><span class="default">?&gt;<br /></span><br />The result:<br /><br />A_obj info: eeee<br />B_obj info: eeee<br />B_obj info after change: xxxxx<br />A_obj info: xxxxx</span></code></div>
  ...
  <strong class="user"><em>info at comfortnet dot de</em></strong></a><a class="genanchor" href="#130363"> &para;</a><div class="date" title="2025-06-11 12:14"><strong>3 months ago</strong></div>
  <div class="text" id="Hcom130363">
<div class="phpcode"><code><span class="html">You should be careful when using pointers. The following example shows how an array is unintentionally distorted if the pointer variable is not treated with unset<br /><br /><span class="default">&lt;?php<br /><br /></span><span class="keyword">echo </span><span class="string">'&lt;pre&gt;'</span><span class="keyword">;<br /><br /></span><span class="default">$arr </span><span class="keyword">= array ( </span><span class="string">'a' </span><span class="keyword">=&gt; </span><span class="string">'a' </span><span class="keyword">, </span><span class="string">'b' </span><span class="keyword">=&gt; </span><span class="string">'b' </span><span class="keyword">) ;<br /><br /></span><span class="default">var_dump </span><span class="keyword">(</span><span class="default">$arr</span><span class="keyword">) ;<br /><br />foreach ( </span><span class="default">$arr </span><span class="keyword">as </span><span class="default">$index </span><span class="keyword">=&gt; &amp;</span><span class="default">$data </span><span class="keyword">) {}<br /><br /></span><span class="default">var_dump </span><span class="keyword">(</span><span class="default">$arr</span><span class="keyword">) ;<br /><br /></span><span class="comment">// unset($data);<br /><br /></span><span class="keyword">foreach ( </span><span class="default">$arr </span><span class="keyword">as </span><span class="default">$index </span><span class="keyword">=&gt; </span><span class="default">$data </span><span class="keyword">) {}<br /><br /></span><span class="default">var_dump </span><span class="keyword">(</span><span class="default">$arr</span><span class="keyword">) ;<br /><br /></span><span class="default">?&gt;<br /></span><br />Output:<br /><br />array(2) {<br />  ["a"]=&gt;<br />  string(1) "a"<br />  ["b"]=&gt;<br />  string(1) "b"<br />}<br />array(2) {<br />  ["a"]=&gt;<br />  string(1) "a"<br />  ["b"]=&gt;<br />  &amp;string(1) "b"<br />}<br />array(2) {<br />  ["a"]=&gt;<br />  string(1) "a"<br />  ["b"]=&gt;<br />  &amp;string(1) "a"<br />}</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">After some headaches, here is a function to check if, between 2 variables $a,$b check if one is a reference to the other.<br />It means they "point" to the same value.<br />Tested on :<br />PHP 7.2.2 (cli) (built: Jan 31 2018 19:31:17) ( ZTS MSVC15 (Visual C++ 2017) x64 )<br />Hope that helps...<br /><span class="default">&lt;?php<br /><br /></span><span class="keyword">function </span><span class="default">are_references</span><span class="keyword">(&amp;</span><span class="default">$a</span><span class="keyword">, &amp;</span><span class="default">$b</span><span class="keyword">){<br />    </span><span class="default">$mem </span><span class="keyword">= </span><span class="default">$a</span><span class="keyword">;     </span><span class="comment">//memorize<br />    </span><span class="default">$a </span><span class="keyword">= </span><span class="default">uniqid </span><span class="keyword">(</span><span class="string">"REFERENCE???"</span><span class="keyword">, </span><span class="default">true </span><span class="keyword">); </span><span class="comment">//change $a<br />    </span><span class="default">$same </span><span class="keyword">= </span><span class="default">$a </span><span class="keyword">=== </span><span class="default">$b</span><span class="keyword">; </span><span class="comment">//compare<br />    </span><span class="default">$a </span><span class="keyword">= </span><span class="default">$mem</span><span class="keyword">; </span><span class="comment">//restore $a<br />    </span><span class="keyword">return </span><span class="default">$same</span><span class="keyword">;<br />}<br />echo </span><span class="string">"***distinct vars AND distinct values\n"</span><span class="keyword">;<br /></span><span class="default">$a </span><span class="keyword">= </span><span class="string">"toto"</span><span class="keyword">;<br /></span><span class="default">$b </span><span class="keyword">= </span><span class="string">"tata"</span><span class="keyword">;<br /><br /></span><span class="default">var_dump</span><span class="keyword">(</span><span class="default">$a</span><span class="keyword">, </span><span class="default">$b</span><span class="keyword">, </span><span class="default">are_references</span><span class="keyword">(</span><span class="default">$a</span><span class="keyword">, </span><span class="default">$b</span><span class="keyword">));<br />echo </span><span class="string">"verify original values: </span><span class="default">$a</span><span class="string">, </span><span class="default">$b</span><span class="string">\n"</span><span class="keyword">;<br /><br />echo </span><span class="string">"***distinct vars BUT SAME values\n"</span><span class="keyword">;<br /></span><span class="default">$a </span><span class="keyword">= </span><span class="string">"toto"</span><span class="keyword">;<br /></span><span class="default">$b </span><span class="keyword">= </span><span class="string">"toto"</span><span class="keyword">;<br /><br /></span><span class="default">var_dump</span><span class="keyword">(</span><span class="default">$a</span><span class="keyword">, </span><span class="default">$b</span><span class="keyword">, </span><span class="default">are_references</span><span class="keyword">(</span><span class="default">$a</span><span class="keyword">, </span><span class="default">$b</span><span class="keyword">));<br />echo </span><span class="string">"verify original values: </span><span class="default">$a</span><span class="string">, </span><span class="default">$b</span><span class="string">\n"</span><span class="keyword">;<br /><br />echo </span><span class="string">'*** $b is a reference of $a'</span><span class="keyword">.</span><span class="string">"\n"</span><span class="keyword">;<br /></span><span class="default">$a </span><span class="keyword">= </span><span class="string">"titi"</span><span class="keyword">;<br /></span><span class="default">$b </span><span class="keyword">= &amp;</span><span class="default">$a</span><span class="keyword">;<br /><br /></span><span class="default">var_dump</span><span class="keyword">(</span><span class="default">$a</span><span class="keyword">, </span><span class="default">$b</span><span class="keyword">, </span><span class="default">are_references</span><span class="keyword">(</span><span class="default">$a</span><span class="keyword">, </span><span class="default">$b</span><span class="keyword">));<br />echo </span><span class="string">"verify original values: </span><span class="default">$a</span><span class="string">, </span><span class="default">$b</span><span class="string">\n"</span><span class="keyword">;<br /><br />echo </span><span class="string">'*** $a is a reference of $b'</span><span class="keyword">.</span><span class="string">"\n"</span><span class="keyword">;<br /></span><span class="default">$b </span><span class="keyword">= </span><span class="string">"titi"</span><span class="keyword">;<br /></span><span class="default">$a </span><span class="keyword">= &amp;</span><span class="default">$b</span><span class="keyword">;<br /><br /></span><span class="default">var_dump</span><span class="keyword">(</span><span class="default">$a</span><span class="keyword">, </span><span class="default">$b</span><span class="keyword">, </span><span class="default">are_references</span><span class="keyword">(</span><span class="default">$a</span><span class="keyword">, </span><span class="default">$b</span><span class="keyword">));<br />echo </span><span class="string">"verify original values: </span><span class="default">$a</span><span class="string">, </span><span class="default">$b</span><span class="string">\n"</span><span class="keyword">;<br /></span><span class="default">?&gt;<br /></span>Result:<br />***distinct vars AND distinct values<br />string(4) "toto"<br />string(4) "tata"<br />bool(false)<br />verify original values: toto, tata<br />***distinct vars BUT SAME values<br />string(4) "toto"<br />string(4) "toto"<br />bool(false)<br />verify original values: toto, toto<br />*** $b is a reference of $a<br />string(4) "titi"<br />string(4) "titi"<br />bool(true)<br />verify original values: titi, titi<br />*** $a is a reference of $b<br />string(4) "titi"<br />string(4) "titi"<br />bool(true)<br />verify original values: titi, titi</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">To change values in a multi-dimensional array while looping through:<br /><br />$var=array('a'=&gt;array(1,2,3),'b'=&gt;array(4,5,6));<br /><br />foreach ($var as &amp;$sub) {<br />  foreach ($sub as &amp;$element) {<br />    $element=$element+1;<br />  }<br />}<br /><br />var_dump($var);<br /><br />------------------------------<br />produces:<br />------------------------------<br />array(2) {<br />  ["a"]=&gt;<br />  array(3) {<br />    [0]=&gt;<br />    int(2)<br />    [1]=&gt;<br />    int(3)<br />    [2]=&gt;<br />    int(4)<br />  }<br />  ["b"]=&gt;<br />  array(3) {<br />    [0]=&gt;<br />    int(5)<br />    [1]=&gt;<br />    int(6)<br />    [2]=&gt;<br />    int(7)<br />  }<br />}</span></code></div>
  </div>
 </div></div>
