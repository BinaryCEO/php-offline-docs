...
<div class="phpcode"><code><span class="html">function run_process($cmd,$outputFile = '/dev/null', $append = false){<br />                        $pid=0;<br />                    if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {//'This is a server using Windows!';<br />                            $cmd = 'wmic process call create "'.$cmd.'" | find "ProcessId"';<br />                            $handle = popen("start /B ". $cmd, "r");<br />                            $read = fread($handle, 200); //Read the output <br />                            $pid=substr($read,strpos($read,'=')+1);<br />                            $pid=substr($pid,0,strpos($pid,';') );<br />                            $pid = (int)$pid;<br />                            pclose($handle); //Close<br />                    }else{<br />                        $pid = (int)shell_exec(sprintf('%s %s %s 2&gt;&amp;1 &amp; echo $!', $cmd, ($append) ? '&gt;&gt;' : '&gt;', $outputFile));<br />                    }<br />                        return $pid;<br />                }<br />                function is_process_running($pid){<br />                    if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {//'This is a server using Windows!';<br />                            //tasklist /FI "PID eq 6480"<br />                        $result = shell_exec('tasklist /FI "PID eq '.$pid.'"' );<br />                        if (count(preg_split("/\n/", $result)) &gt; 0 &amp;&amp; !preg_match('/No tasks/', $result)) {<br />                            return true;<br />                        }<br />                    }else{<br />                        $result = shell_exec(sprintf('ps %d 2&gt;&amp;1', $pid));<br />                        if (count(preg_split("/\n/", $result)) &gt; 2 &amp;&amp; !preg_match('/ERROR: Process ID out of range/', $result)) {<br />                            return true;<br />                        }<br />                    }<br />                    return false;<br />                }<br />                function stop_process($pid){<br />                        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {//'This is a server using Windows!';<br />                                $result = shell_exec('taskkill /PID '.$pid );<br />                            if (count(preg_split("/\n/", $result)) &gt; 0 &amp;&amp; !preg_match('/No tasks/', $result)) {<br />                                return true;<br />                            }<br />                        }else{<br />                                $result = shell_exec(sprintf('kill %d 2&gt;&amp;1', $pid));<br />                            if (!preg_match('/No such process/', $result)) {<br />                                return true;<br />                            }<br />                        }<br />                }<br />                    $cmd='';<br />                    if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {//'This is a server using Windows!';<br />                         $cmd=  $php_path.'\php.exe '.$path.'\long_process.php' ;    <br />                    }else{<br />                        $cmd='/usr/bin/php -f  /var/www/example.com/public/long_process.php';<br />                    }<br /><br />$pid=run_process($cmd);</span></code></div>
  ...
  <strong class="user"><em>raitech at gmail dot com</em></strong></a><a class="genanchor" href="#118303"> &para;</a><div class="date" title="2015-11-12 10:47"><strong>9 years ago</strong></div>
  <div class="text" id="Hcom118303">
<div class="phpcode"><code><span class="html">This method seems only useful when you doesn't need to know something about the worker, when you can let runTasks() finishes its code and destroy the GearmanClient object without a problem.<br /><br />If you need to get data from worker via callbacks, forget it after runTasks() finishes.<br /><br />You have to block your execution in the GearmanClient "main loop", so it can call the callbacks. It seems that runTasks() is that "main loop".</span></code></div>
  </div>
 </div></div>
