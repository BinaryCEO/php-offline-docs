...
<head>
 ...
 <link rel="canonical" href="https://www.php.net/manual/en/memcached.addservers.php">
 <link rel="shorturl" href="https://www.php.net/manual/en/memcached.addservers.php">
 <link rel="alternate" href="https://www.php.net/manual/en/memcached.addservers.php" hreflang="x-default">

 <link rel="contents" href="https://www.php.net/manual/en/index.php">
 <link rel="index" href="https://www.php.net/manual/en/class.memcached.php">
 <link rel="prev" href="https://www.php.net/manual/en/memcached.addserver.php">
 <link rel="next" href="https://www.php.net/manual/en/memcached.append.php">
      ...
      <form action="/manual/change.php" method="get" id="changelang" name="changelang">
        <fieldset>
          ...
          <select onchange="document.changelang.submit()" name="page" id="changelang-langs">
            <option value='en/memcached.addservers.php' selected="selected">English</option>
            ...
            <option value='tr/memcached.addservers.php'>Turkish</option>
 ...
 <div class="refsect1 examples" id="refsect1-memcached.addservers-examples">
  ...
  <p class="para">
   <div class="example" id="example-4672">
    <p><strong>Example #1 <span class="function"><strong>Memcached::addServers()</strong></span> example</strong></p>
   ...
   <ul class="simplelist">
    ...
    <li><span class="methodname"><a href="memcached.resetserverlist.php" class="methodname" rel="rdfs-seeAlso">Memcached::resetServerList()</a> - Clears all servers from the server list</span></li>
      ...
      <div class="edit-bug">
        ...
        •
        <a href="https://github.com/php/doc-en/blob/master/reference/memcached/memcached/addservers.xml">Submit a Pull Request</a>
        •
        <a href="https://github.com/php/doc-en/issues/new?body=From%20manual%20page:%20https:%2F%2Fphp.net%2Fmemcached.addservers%0A%0A---">Report a Bug</a>
      </div>
    </div><section id="usernotes">
 <div class="head">
  <span class="action"><a href="/manual/add-note.php?sect=memcached.addservers&amp;repo=en&amp;redirect=https://www.php.net/manual/en/memcached.addservers.php">＋<small>add a note</small></a></span>
...
<div class="phpcode"><code><span class="html">For a proper FailOver mechanism:<br /><br />$memcached = new Memcached();<br />$memcached-&gt;setOption(Memcached::OPT_CONNECT_TIMEOUT, 10);<br />$memcached-&gt;setOption(Memcached::OPT_DISTRIBUTION, Memcached::DISTRIBUTION_CONSISTENT);<br />$memcached-&gt;setOption(Memcached::OPT_SERVER_FAILURE_LIMIT, 2);<br />$memcached-&gt;setOption(Memcached::OPT_REMOVE_FAILED_SERVERS, true);<br />$memcached-&gt;setOption(Memcached::OPT_RETRY_TIMEOUT, 1);<br />$memcached-&gt;addServers($servers);<br /><br />Memcached::OPT_DISTRIBUTION: set it to consistent hashing. If one memcached node is dead, its keys (and only its keys) will be evenly distributed to other nodes. This is where the magic is done. This is really different from removing one server in your -&gt;addServers() call.<br /><br />Memcached::OPT_SERVER_FAILURE_LIMIT: number of connection issues before a server is marked as DEAD, and removed from the list of servers (default: 5).<br /><br />Memcached::OPT_REMOVE_FAILED_SERVERS: set it to «true», to enable the removal of dead servers.<br /><br />Memcached::OPT_RETRY_TIMEOUT: after a node is declared DEAD, libmemcached will try it again after that many seconds. Here I've set it to 1 second, but I'm working on PHP scripts that run for less than 100ms most of the time. That would only be useful for cron/daemonize scripts.<br /><br />Memcached::OPT_CONNECT_TIMEOUT: the timeout after which a server is considered DEAD. As my servers are on the same LAN, ping is ~0.5ms, so 10ms is large enough to consider the server is DEAD. Note that you have to wait twice that time before a node is marked DEAD, so if it's 1000ms, your script will lock for 2 seconds before ignoring the DEAD server. That may affect your response times a lot, and that's why I've set it very low<br /><br />Author of this is Yvan from Dugwood</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">See the note for __construct()<br /><br />Important to not call -&gt;addServers() every run -- only call it if no servers exist (check getServerList() ); otherwise, since addServers() does not check for dups, it will let you add the same server again and again and again, resultings in hundreds if not thousands of connections to the MC daemon.</span></code></div>
  </div>
 </div></div>
<div class="foot"><a href="/manual/add-note.php?sect=memcached.addservers&amp;repo=en&amp;redirect=https://www.php.net/manual/en/memcached.addservers.php">＋<small>add a note</small></a></div>
