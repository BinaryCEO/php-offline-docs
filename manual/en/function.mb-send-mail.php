...
<head>
 ...
 <link rel="prev" href="https://www.php.net/manual/en/function.mb-scrub.php">
 <link rel="next" href="https://www.php.net/manual/en/function.mb-split.php">

 <link rel="alternate" href="https://www.php.net/manual/en/function.mb-send-mail.php" hreflang="en">
 <link rel="alternate" href="https://www.php.net/manual/de/function.mb-send-mail.php" hreflang="de">
 <link rel="alternate" href="https://www.php.net/manual/es/function.mb-send-mail.php" hreflang="es">
 <link rel="alternate" href="https://www.php.net/manual/fr/function.mb-send-mail.php" hreflang="fr">
 <link rel="alternate" href="https://www.php.net/manual/it/function.mb-send-mail.php" hreflang="it">
 <link rel="alternate" href="https://www.php.net/manual/ja/function.mb-send-mail.php" hreflang="ja">
 <link rel="alternate" href="https://www.php.net/manual/pt_BR/function.mb-send-mail.php" hreflang="pt_BR">
 <link rel="alternate" href="https://www.php.net/manual/ru/function.mb-send-mail.php" hreflang="ru">
 <link rel="alternate" href="https://www.php.net/manual/tr/function.mb-send-mail.php" hreflang="tr">
 <link rel="alternate" href="https://www.php.net/manual/uk/function.mb-send-mail.php" hreflang="uk">
 <link rel="alternate" href="https://www.php.net/manual/zh/function.mb-send-mail.php" hreflang="zh">
...
<div id="layout" class="clearfix">
 ...
 <div class="head">
  <span class="action"><a href="/manual/add-note.php?sect=function.mb-send-mail&amp;repo=en&amp;redirect=https://www.php.net/manual/en/function.mb-send-mail.php">＋<small>add a note</small></a></span>
  ...
  <strong class="user"><em>gordon at kanazawa-gu dot ac dot jp</em></strong></a><a class="genanchor" href="#27998"> &para;</a><div class="date" title="2002-12-28 06:07"><strong>22 years ago</strong></div>
  <div class="text" id="Hcom27998">
<div class="phpcode"><code><span class="html">If your server doesn't have mb_send_mail() enabled but you want to use non-ascii (multi-byte) chars in an email's subject or name headers, you can use something like the following:<br /><br />$charset = "iso-2202-jp"; // japanese<br />$to = encode("japanese name 01", $charset) . " &lt;to@email.com&gt;";<br />$from = encode("japanese name 02", $charset) . " &lt;from@email.com&gt;";<br />$subject = encode("japanese text", $charset);<br />$message = "does not need to be encoded";<br />mail($to, $subject, $message, $from);<br /><br />function encode($in_str, $charset) {<br />    $out_str = $in_str;<br />    if ($out_str &amp;&amp; $charset) {<br /><br />        // define start delimimter, end delimiter and spacer<br />        $end = "?=";<br />        $start = "=?" . $charset . "?B?";<br />        $spacer = $end . "\r\n " . $start;<br /><br />        // determine length of encoded text within chunks<br />        // and ensure length is even<br />        $length = 75 - strlen($start) - strlen($end);<br />        $length = floor($length/2) * 2;<br /><br />        // encode the string and split it into chunks <br />        // with spacers after each chunk<br />        $out_str = base64_encode($out_str);<br />        $out_str = chunk_split($out_str, $length, $spacer);<br /><br />        // remove trailing spacer and <br />        // add start and end delimiters<br />        $spacer = preg_quote($spacer);<br />        $out_str = preg_replace("/" . $spacer . "$/", "", $out_str);<br />        $out_str = $start . $out_str . $end;<br />    }<br />    return $out_str;<br />}<br />// for details on Message Header Extensions <br />// for Non-ASCII Text see ...<br />// <a href="http://www.faqs.org/rfcs/rfc2047.html" rel="nofollow" target="_blank">http://www.faqs.org/rfcs/rfc2047.html</a></span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html"># self-fix...<br />I posted "encode_mimeheader" workaround on the day before. But I found that the code depends on platforms. :(<br /><br />In some platforms, hedders(after the header splited into two or more lines) will appear in the body content.<br />The cause is that 'there are some platforms where the translation from \n to \r\n is apparently done for you'.<br /># See the post by "leon at ietsmet dot nl" about mail function (23-Apr-2003).<br /><br />Now, all you have to do is just changing the glue string from "\r\n " to "\n ".<br /><br />function encode_mimeheader($str, $indent = 0, $encoding = 'utf-8', $mail_encoding = 'iso-2022-jp')<br />{<br />         ..... lefted out (all same).....<br />  $str = join("\r\n ", $lines); // RFC 2047,822 say newline must be ^\r\n, not only\n<br />  return $str;<br />}<br /><br />should be below in some platform...<br /><br />function encode_mimeheader($str, $indent = 0, $encoding = 'utf-8', $mail_encoding = 'iso-2022-jp')<br />{<br />         ..... lefted out (all same).....<br />  $str = join("\n ", $lines);<br />  // Though RFC 2047,822 say newline must be \r\n, not only \n,<br />  // in my platform, auto replacement will be done (accepts \n and \n\r\n -&gt; \n\n)...<br />  return $str;<br />}<br /><br /># notice: In both code, 1 space after newline is essential (gule str is *not* only "\r\n" nor "\n").<br /># This space is ignored only between encoded-words.<br /><br />    c.f. about efficiency<br /><br />Thinking about efficiency, my encode_mimeheader may have to determine where to split string into lines without mb_encode_mimeheader in the determining loop. But that loop will be repeated only one or a few times and this function  will be usually used against not so long string. So, this loss is not critical in most case.<br />If you use only one encoding, you can easily check ascii or multi-byte char one by one accoding to the definition of the encoding and determining splitting point (this is far faster), but when you have to handle all encoding that is not so easy. At least, I don't want to touch. :p<br />To accept *any* encoding (encoding name will be passed as argument), you can use this code. This is because I post non-efficient code like this.<br /># Though I'm not sure whether the code actually work well with all encoding...</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">I have to use Japanese hankaku (single byte kana in (S)JIS) in mail subject and message,<br />so I tried the way that NO3L in Japan said, but I found there's a big problem.<br />In that way, mb_encode_mimeheader drop escape sequence from the head of encoded string from second line.<br />if you use long letter in subject, and if it includes multi byte string, It must crashed.<br />Therefore, if you want to use Japanese hankaku in mail subject, you might try like this.<br />Encoded subject letters in one line is not just 76 letters, so it is not based on RFC, but it work.<br /><br />$intSubjectLength  = mb_strlen($strSubject);<br />$intSeparateLength = 10;<br />for ($i=0; $i&lt;ceil($intSubjectLength / $intSeparateLength); $i++) {<br />    $arrSeparatedSubject[$i] = mb_substr($strSubject, $intIndex, $intSeparateLength);<br />    $arrSeparatedSubject[$i] = mb_encode_mimeheader(mb_convert_encoding($arrSeparatedSubject[$i], "JIS", "EUC-JP"));<br />    $intIndex = $intIndex + $intSeparateLength;<br />}<br />$strSubject = join("\n ", $arrSeparatedSubject);</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">Make sure that if you're using a form to type in the mail, that your form page has the right encoding, like if I want to send out a japanese email, by filling out a form, the form page needs this in the header:<br />&lt;META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=SHIFT-JIS"&gt;</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">I was trying to send japanese email and had a hell of a time getting it to work. I finally stumbled across mb_send_mail() and it did everything I wanted *except* that it only send plain text emails ... sending HTML contents was impossible.<br /><br />I finally whipped this up. I'm posting in case someone finds it useful.<br /><br />function send_japanese_mail($to, $subject, $body, $from, $from_email, $is_html_content=false) {<br /><br />   $headers  = "MIME-Version: 1.0\\n" ;<br />   $headers .= "From: $from &lt;$from_email&gt;\\n";<br />   $headers .= "Reply-To: $from &lt;$from_email&gt;\\n";<br /> <br />  /* If the body  is HTML or plain text set the Content-Type header accordingly */<br /><br />   if ($is_html_content) {<br />     $headers .= "Content-Type: text/html;charset=ISO-2022-JP\\n";<br />     /* turn all line breaks into BR tags */<br />     $body = nl2br($body);<br />   }<br />     else {<br />     $headers .= "Content-Type: text/plain;charset=ISO-2022-JP\\n";<br />   }<br /><br />  /* need to convert body to same encoding as stated in Content-Type header above */<br /><br />  $body = mb_convert_encoding($body, "ISO-2022-JP","AUTO");<br /><br />  /* set any sendmail params, optional ... */<br />  $sendmail_params  = "-f$from_email";<br /><br />  /*<br />   The subject is actually a "header" and can/will get mangled<br />   if it contains non-ASCII characters. So we need to convert<br />   the subject to something containing only ASCII characters.<br /><br />   First we convert the subject to the same encoding as the<br />   body, then we use mb_encode_mimeheader() to make the subject<br />   line all ASCII characters.<br />  */<br /><br />   mb_language("ja");<br />   $subject = mb_convert_encoding($subject, "ISO-2022-JP","AUTO");<br />   $subject = mb_encode_mimeheader($subject);<br /><br />   mail($to, $subject, $body, $headers, $sendmail_params);<br />}</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">Tested with PHP 4.2.2 on Linux: Please note that if you're using Unicode (mb_language("uni")) and you attempt to send mail with mb_send_mail(), you will need to base64_encode() the message body - mb_send_mail() doesn't do that for you. It does, however, issue the correct message headers, so you don't need to worry about that.<br /><br />Also note that neither mb_language() nor mb_send_mail() is able to convert your message to UTF-8. *You* need to provide the UTF-8-encoded (and base64-encoded) message, and then mb_send_mail() will issue the correct message headers.<br /><br />Here's an example of sending an UTF-8-encoded message with mb_send_mail():<br /><br />mb_language("uni");<br />$body = chunk_split(base64_encode("International characters"));<br />mb_send_mail("someone@example.com", "Subject", $body);<br /><br />If the receiving mail client supports UTF-8 properly, you will be able to send messages that contain a mix of all kinds of characters (e.g., you could send Thai, Chinese, and Danish characters in the same message). Not all mail clients support UTF-8, though. At the time of writing, some of the more popular Windows email clients - Eudora and Pegasus Mail - don't. There are a number of email clients with working support for UTF-8. These include Outlook Express, KMail, Mozilla, Netscape 6/7, Sylpheed, Evolution and others.</span></code></div>
  ...
  <strong class="user"><em>Anonymous</em></strong></a><a class="genanchor" href="#95753"> &para;</a><div class="date" title="2010-01-20 03:20"><strong>15 years ago</strong></div>
  <div class="text" id="Hcom95753">
<div class="phpcode"><code><span class="html">If you have problem using mb_send_mail to send utf-8 mails, you have to know the following things (I've spent hours on the net before finding the correct php code) :<br /><br />- You have to use the mb_encode_mimeheader function on the sender name and on the recipient name (on names, not on emails !).<br />- Subject and message are automatically encoded.<br />- Add - at least - the following header :<br /><br />$headers = "Mime-Version: 1.0\n";<br />$headers .= "Content-Type: text/plain;charset=UTF-8\n";<br />$headers .= "From: $sender";<br /><br />- Last but not least, beware of the internal encoding, which is needed by mb_encode_mimeheader function in order to encode properly. I had to set the internal encoding to UTF-8 in order to make it work properly :<br /><br />mb_internal_encoding("UTF-8");</span></code></div>
  </div>
 </div>
  ...
  <strong class="user"><em>contact-form at contactrobot dot com</em></strong></a><a class="genanchor" href="#111002"> &para;</a><div class="date" title="2013-01-04 11:52"><strong>12 years ago</strong></div>
  <div class="text" id="Hcom111002">
<div class="phpcode"><code><span class="html">While using the right tags mentioned in other comments (mb_internal_encoding, Content-Type, etc) ...<br /><br />mb_send_mail would properly encode special characters in plain text emails, for both subject and message<br /><br />mb_send_mail would not properly encode special characters in subject for multi-part messages (text+html)</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">Easy and simple....<br />Send an email with Japanese contain.<br /><br /><span class="default">&lt;?php<br /><br /></span><span class="comment">/**<br />     * @name                    : sendMail<br />     * @author Taslim Mazumder Sohel<br />     * @mailsohel62@yahoo.com<br />     * Function for sending email <br />     *with Japanese Email Body, Subject, Sender Name.<br />     *<br />     * @param String $to               : Receiver mail address.<br />     * @param String $subject          : email subject.<br />     * @param int    $body            : mail body text.<br />     * @param array  $from_email    : Sender mail address.<br />     * @param array  $from_name      : Sender Name.<br />     *<br />     */<br /></span><span class="keyword">function </span><span class="default">sendMail</span><span class="keyword">(</span><span class="default">$to</span><span class="keyword">, </span><span class="default">$subject</span><span class="keyword">, </span><span class="default">$body</span><span class="keyword">, </span><span class="default">$from_email</span><span class="keyword">,</span><span class="default">$from_name</span><span class="keyword">)<br /> {<br /></span><span class="default">$headers  </span><span class="keyword">= </span><span class="string">"MIME-Version: 1.0 \n" </span><span class="keyword">;<br /></span><span class="default">$headers </span><span class="keyword">.= </span><span class="string">"From: " </span><span class="keyword">.<br />       </span><span class="string">""</span><span class="keyword">.</span><span class="default">mb_encode_mimeheader </span><span class="keyword">(</span><span class="default">mb_convert_encoding</span><span class="keyword">(</span><span class="default">$from_name</span><span class="keyword">,</span><span class="string">"ISO-2022-JP"</span><span class="keyword">,</span><span class="string">"AUTO"</span><span class="keyword">)) .</span><span class="string">"" </span><span class="keyword">.<br />       </span><span class="string">"&lt;"</span><span class="keyword">.</span><span class="default">$from_email</span><span class="keyword">.</span><span class="string">"&gt; \n"</span><span class="keyword">;<br /></span><span class="default">$headers </span><span class="keyword">.= </span><span class="string">"Reply-To: " </span><span class="keyword">.<br />       </span><span class="string">""</span><span class="keyword">.</span><span class="default">mb_encode_mimeheader </span><span class="keyword">(</span><span class="default">mb_convert_encoding</span><span class="keyword">(</span><span class="default">$from_name</span><span class="keyword">,</span><span class="string">"ISO-2022-JP"</span><span class="keyword">,</span><span class="string">"AUTO"</span><span class="keyword">)) .</span><span class="string">"" </span><span class="keyword">.<br />       </span><span class="string">"&lt;"</span><span class="keyword">.</span><span class="default">$from_email</span><span class="keyword">.</span><span class="string">"&gt; \n"</span><span class="keyword">;<br /> <br />    <br /></span><span class="default">$headers </span><span class="keyword">.= </span><span class="string">"Content-Type: text/plain;charset=ISO-2022-JP \n"</span><span class="keyword">;<br /><br />    <br /></span><span class="comment">/* Convert body to same encoding as stated <br />in Content-Type header above */<br />    <br /></span><span class="default">$body </span><span class="keyword">= </span><span class="default">mb_convert_encoding</span><span class="keyword">(</span><span class="default">$body</span><span class="keyword">, </span><span class="string">"ISO-2022-JP"</span><span class="keyword">,</span><span class="string">"AUTO"</span><span class="keyword">);<br />    <br /></span><span class="comment">/* Mail, optional paramiters. */<br /></span><span class="default">$sendmail_params  </span><span class="keyword">= </span><span class="string">"-f</span><span class="default">$from_email</span><span class="string">"</span><span class="keyword">;<br />    <br /></span><span class="default">mb_language</span><span class="keyword">(</span><span class="string">"ja"</span><span class="keyword">);<br /></span><span class="default">$subject </span><span class="keyword">= </span><span class="default">mb_convert_encoding</span><span class="keyword">(</span><span class="default">$subject</span><span class="keyword">, </span><span class="string">"ISO-2022-JP"</span><span class="keyword">,</span><span class="string">"AUTO"</span><span class="keyword">);<br /></span><span class="default">$subject </span><span class="keyword">= </span><span class="default">mb_encode_mimeheader</span><span class="keyword">(</span><span class="default">$subject</span><span class="keyword">);<br /><br /></span><span class="default">$result </span><span class="keyword">= </span><span class="default">mail</span><span class="keyword">(</span><span class="default">$to</span><span class="keyword">, </span><span class="default">$subject</span><span class="keyword">, </span><span class="default">$body</span><span class="keyword">, </span><span class="default">$headers</span><span class="keyword">, </span><span class="default">$sendmail_params</span><span class="keyword">);<br />       <br />return </span><span class="default">$result</span><span class="keyword">;<br />}<br />        <br /></span><span class="default">?&gt;</span></span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">There is another way of encoding Japanese language to send email without worrying about mb_functions:<br /><br />Download PHP Jcode from <a href="http://www.spencernetwork.org/" rel="nofollow" target="_blank">http://www.spencernetwork.org/</a><br />and use JcodeConvert function with base64_encode function for the e-mail header and the subject.<br /><br />$email_to = "blah@blah.com";<br /><br />$_POST["name"] = strip_tags(trim($_POST["name"]));<br />$name = "=?ISO-2022-JP?B?". base64_encode(JcodeConvert($_POST["name"], 2, 3)). "?=";<br />$_POST["email"] = strip_tags(trim($_POST["email"]));<br />$_POST["subject"] = strip_tags(trim($_POST["subject"]));<br />$email_subject = "=?ISO-2022-JP?B?". base64_encode(JcodeConvert($_POST["subject"], 2, 3)). "?=";<br />$_POST["body"] = strip_tags(trim($_POST["body"]));<br /><br />$email_header = "From: $name &lt;". $_POST["email"] ."&gt;\n";<br />$email_header .= "Reply-To: ". $_POST["email"] ."\n";<br />$email_header .= "Content-transfer-encoding: 7bit\n";<br />$email_header .= "Content-type: text/plain; charset=\"iso-2022-jp\"\n\n";<br /><br />$email_body = "blah blah blah.\n"<br /><br />mail($email_to, $email_subject, $email_body, $email_header);</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">As others say, mb_encode_mimeheader() seems to have a bug with JIS(ISO-2022-JP) encording. Token indicating start and end of multibyte char is inserted only before start of encoding and after the all.<br />We need the tokens at start and end of *all* encoded-text.<br /># PHP 4.3.2<br /><br />So, we needs some workaround.<br /><br />The post by "gordon at kanazawa-gu dot ac dot jp" seems to work well, but it doesn't. JIS nees some special tokens and base64_encode does'nt output them, so the code cannot used.<br /><br />The post "RE: N03L in Japan", which simply splitting words in eash 10 chars is good enough in most case, there are some problem left. When there are some splited parts starting with ascii chars, 1 additional space will be inserted.<br /># RFC2047 only say within 76 words, shorter is not bad.<br /><br />Additionally, thought it is really rare case, when we use "=?charset?" as literal string, we have to escape them.<br /># Some mailer can actually send this without escape and header will be broken. :(<br /><br />Now, a little non-smart but maybe more accurate code is below function. I tested only with ISO-2022-JP, only in costomized phpBB2.0.5, only some cases.<br />As far as my test this code worked well, but I'm not sure.<br /><br /># $str: source text<br /># $indent: ex. for "Subject: " header, give 9 and first line will be shorter than 76-1-9=66<br /># $encoding: source text encoding<br /># $mail_encoding: $str will be converted into this before base64 encode<br /><br />function encode_mimeheader($str, $indent = 0, $encoding = 'utf-8', $mail_encoding = 'iso-2022-jp')<br />{<br />  $start_delimiter = strtoupper("=?$mail_encoding?B?");<br />  $start_pattern   = strtoupper("=\\?$mail_encoding\\?B\\?");<br />  $end_delimiter   = '?=';<br /><br />  $str = mb_convert_encoding($str, $mail_encoding, $encoding);<br />  $length  = mb_strlen($str, $mail_encoding);<br />  $max_part_length = 20; // enough short in most case (you can change this default value)<br />    <br />  for ($i=0, $index=0; $index&lt;$length; $i++) {<br />    $part_length = $max_part_length;<br />    $s = mb_substr($str, $index, $part_length, $mail_encoding);<br />    // workaround for literally used start delimiter (without below, subject may break)<br />    // note: mb_encode_mimeheader() don't encode ascii including start delimiter<br />    if (preg_match('/^' . $start_pattern . '/i', $s))<br />    {<br />      $lines[$i] = $start_delimiter . base64_encode($start_delimiter) . "?=";<br />      $index += strlen($start_delimiter);<br />      continue;<br />    }<br /><br />    $lines[$i] = mb_encode_mimeheader($s, $mail_encoding);<br />    while (strlen($lines[$i]) &gt; 76-1 - ($i?0:$indent)) { // max par line - first space - indent (first line only)<br />      $part_length = floor($part_length * (76-1 - ($i?0:$indent)) / strlen($lines[$i])); // at least 1 decrement<br />      $s = mb_substr($str, $index, $part_length, $mail_encoding);<br />      $lines[$i] = mb_encode_mimeheader($s, $mail_encoding);<br />    }<br />    <br />    // workaround for starting new line with ascii (without below, 1 space may cut in)<br />    // note: mb_encode_mimeheader() starts encode after encounterring multibyte char<br />    if ($i &gt; 0 &amp;&amp; !preg_match('/^' . $start_pattern . '/i', $lines[$i]))<br />    {<br />      $p = strpos($lines[$i], $start_delimiter); //never 0 (false when not found)<br />      $p = $p ? $p : strlen($lines[$i]);<br />      $lines[$i] = $start_delimiter . base64_encode(substr($lines[$i], 0, $p)) . "?=";<br />      $part_length = $p;<br />    }<br />    $index += $part_length;<br />  }<br />  $str = join("\r\n ", $lines); // RFC 2047,822 say newline must be ^\r\n, not only\n<br />  return $str;<br />}</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">Yes, as far as I understood the function encodes the entrire message body with base64 and that's why it can be used to send messages with attachments.</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">Be careful, mb_send_mail chokes if you try and send a multipart e-mail.</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">You may not use mb_encode_mimeheader() with mb_convert_encoding() to make subject as follows. It causes mojibake in several strings.<br />mb_encode_mimeheader( mb_convert_encoding($strMailSubj, "JIS", "EUC-JP") )<br /><br />Set mb_internal_encoding() to *subject's* encoding and call mb_encode_mimeheader.<br /><br />Example)<br /><br />    $__lang = mb_language();<br />    $__enc = mb_internal_encoding();<br />    mb_language("Japanese");<br />    mb_internal_encoding( mb_detect_encoding($subject) );<br />    #mb_internal_encoding( "EUC-JP" ); #just do when you know encoding of $subject<br />    mail($to,<br />        mb_encode_mimeheader($subject),<br />        mb_convert_encoding($msg,"JIS","AUTO"),$header);<br />    mb_internal_encoding( $__enc );<br />    mb_language($__lang);</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">First of all, don't use nl2br in newer php versions unless you WANT to write xhtml...you'll get &lt;br /&gt; tags.<br /><br />Secondly, you should use JIS, not ISO-2022-JP as the convert TO encoding.  If you use ISO... mb_encode_mimeheader seems to fail. They're the same charset anyway.<br /><br />Now here's why I am really writing.  I just discovered that mb_send_mail and mb_encode_mimeheader cannot support hankaku (single byte kana in (S)JIS) at all.  If you are making apps for the keitai (mobile) market, this won't do.  Therefore, undo your overloading on the mail function and use regular mail and convert everything yourself like this.  This is what we did.  It seems to work for us.  Your mileage may vary.<br /><br />$strMBS = mb_convert_encoding($strMailBodySend, "JIS", "EUC-JP");<br /><br />$strMS  = mb_encode_mimeheader( mb_convert_encoding($strMailSubj, "JIS", "EUC-JP") );<br /><br />mail($strToMail, $strMS, $strMBS, $strFrom);</span></code></div>
  </div>
 </div></div>
<div class="foot"><a href="/manual/add-note.php?sect=function.mb-send-mail&amp;repo=en&amp;redirect=https://www.php.net/manual/en/function.mb-send-mail.php">＋<small>add a note</small></a></div>
