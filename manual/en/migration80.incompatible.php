...
<div class="phpcode"><code><span class="html">The change in string to int comparison mentioned above (e.g. '' == 0 now equates to false) has some other nasty consequences:<br /><br />$a = '';<br /><br />// php 8<br /><br />if ( $a &lt; 0 ) echo 'true'; // echos true<br />if ( $a &lt; -1) echo 'true'; // echos true<br />if ( $a &lt; -100 ) echo 'true'; // echos true<br /><br />// php 7<br /><br />if ( $a &lt; 0 ) echo 'true'; // no output<br />if ( $a &lt; -1) echo 'true'; // no output<br />if ( $a &lt; -100 ) echo 'true'; // no output<br /><br />So in a situation where you may have a web form input and expected an empty value to equate to 0, watch out not only for == 0, != 0, and &lt;= 0 comparisons, but ALL &lt; or &lt;= comparisons to negative integers.</span></code></div>
  </div>
 </div>
...
<div class="phpcode"><code><span class="html">If you have older projects that break with PHP7 to 8 migration due to the loose comparison issue: <br /><br />i.e. if ($a == 0) different behaviour between PHP 7 and PHP 8 <br />(for case like $a = "" or $a = "123foo" and other cases listed at top)<br /><br />replace in old code: <br /><br />if ($a == 0) { .. }  <br /><br />with <br /><br />if (cmp_eq($a, $b)) { .. } <br /><br />Tested with a wide range of scenarios, even against arrays, booleans, file handles, pipe handles, objects, scalars and numbers.<br /><br />So old code still behave like before.  <br />Then both PHP8.x and older PHP up to ver 7.x will give the exact same boolean true or false output for loose comparisons.<br /><br />function cmp_eq($a, $b) {<br />// If both $a and $b are of type strings, compare them as strings<br />if (is_string($a) &amp;&amp; is_string($b)) { return $a == $b; } // may not be === because php says '42' equals '042' true yet '42' === '042' is false.<br /><br />// If both $a and $b are numeric strings, compare them as numbers<br />if (is_numeric($a) &amp;&amp; is_numeric($b)) { return $a == $b; }<br /><br />// If $a is an empty string and $b is 0, or vice versa, return true<br />if (($a === '' &amp;&amp; $b === 0) || ($a === 0 &amp;&amp; $b === '')) { return true; }<br /><br />// If $a is a non-numeric string and $b is 0, or vice versa, return true<br />if ((is_string($a) &amp;&amp; ($a !== '') &amp;&amp; ($b === 0)) || (($a === 0) &amp;&amp; is_string($b) &amp;&amp; ($b !== ''))) {<br />return true;<br />}<br />// special case '123abc' == 123 .. php 7 casts 123abc to 123, then 123 == 123 results in true. lets mimic that.<br />if ((is_string($a) &amp;&amp; ($a !== '') &amp;&amp; (is_numeric($b)) &amp;&amp; ((bool)$b))) {<br />$number = filter_var($a, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION); //"-234.56xyz"<br />return $number == $b;<br />}<br />if (is_numeric($a) &amp;&amp; ((bool)$a) &amp;&amp; is_string($b) &amp;&amp; ($b !== '')) {<br />$number = filter_var($b, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION); //"-234.56xyz"<br />return $a == $number;<br />}<br /><br />// If $a is a number and $b is a non-numeric string, cast $a to string and compare<br />if (is_numeric($a) &amp;&amp; is_string($b)) { return strval($a) == $b; }<br /><br />// If $b is a number and $a is a non-numeric string, cast $b to string and compare<br />if (is_string($a) &amp;&amp; is_numeric($b)) { return $a == strval($b); }<br /><br />// If $a and $b are both non-numeric strings, compare them directly, we should return true if they are the same<br />return $a == $b;<br />} // end func cmp_eq<br /><br />Note: the better way would be to port code to PHP 8, use strict variable typing and rather make use of the === and !== operators.<br /><br />But in some cases having lots of old code to quickly patch, this might help.</span></code></div>
  </div>
 </div></div>
